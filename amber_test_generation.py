...
def write_amber_thread_instruction(output, instruction, instruc_id, num_of_testing_threads, saturation_level):
    ...
    # and call handle_atomic_store
        ...
        handle_atomic_store(output, write_value, saturation_level)
...
def handle_atomic_exchange_branch(output, check_value, exchange_value, instruction_address, number_of_testing_threads,
    ...
    if saturation_level == 0:
        ...
        output.write("\t\tif (atomicExchange(out_buf.x[index], " + exchange_value + ") ==  " + check_value + ") { \n")
...
def handle_amber_check_branch(output, check_value, instruction_address, number_of_testing_threads, saturation_level):
    ...
    if saturation_level == 0:
        ...
        output.write("\t\tif (atomicAdd(out_buf.x[index], 0) == " + check_value + " ) { \n")
...
def handle_atomic_store(output, write_value, saturation_level):
    # perform the operation either at a single memory location or indexed memory location, depending on saturation level
    if saturation_level == 0:
        output.write("\t\tatomicExchange(test.x, " + write_value + ");\n")
    elif saturation_level == 1 or saturation_level == 2:
        output.write("\t\tatomicExchange(out_buf.x[index], " + write_value + ");\n")
...
def write_amber_epilogue(output, workgroups, threads_per_workgroup, saturation_level):
    ...
    if saturation_level == 1 or saturation_level == 2:
        output.write("BUFFER out DATA_TYPE uint32 SIZE 65535 FILL 0\n")
    ...
    if saturation_level == 1 or saturation_level == 2:
        output.write("  BIND BUFFER out AS storage DESCRIPTOR_SET 0 BINDING 1")
    ...
    if saturation_level == 0:
        output.write("EXPECT tester IDX 4 EQ " + str(total_threads) + "\n")
    elif saturation_level == 1 or saturation_level == 2:
...
def generate_amber_test(inputted_file, output_file_name, config=default_config):
    ...
    if saturation_level < 0 or saturation_level > 3:
    ...
    for number, each_thread in enumerate(instructions):
        write_amber_thread_program(output, each_thread, number, num_of_testing_threads, saturation_level)
